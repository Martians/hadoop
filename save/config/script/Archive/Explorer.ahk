...
CheckWindow(Byref window) {
	;log(window.hwnd " -- " window.LocationURL " --" window.locationName "--" window.Name "--" window.FullName)
	return Instr(window.FullName, "Explorer")
}
...
Explorer_GetWindow(hwnd="")
{
	...
	if (class ~= "(Cabinet|Explore)WClass") {
		for window in ComObjCreate("Shell.Application").Windows {
			if (CheckWindow(window) && window.hwnd == hwnd) {
				return window
			}
		}
